...
################ defines the matrix of moves, a mnemonic based on old notation which I have to update
...
# shuffle of the edges

  node_fiction="abcdefghijklmnoprstuv";
  
  node_block[node_fiction]++;

  for (ke in all_edge_source) {
  
  ebon=all_edge_bond[ke]; if (ebon==bond_ext) { 

     perema=int(1000000 * rand()); 
     shif[perema]=ke;
...
for (ko in shif) {       kot=shif[ko];

     etarg=all_edge_target[kot];
     esour=all_edge_source[kot];

     etarl=all_edge_int[etarg];
     esourl=all_edge_int[esour];
  ...
  for (hu in node_block) {delete node_block[hu];}
  
    for (huo in shif) {delete shif[huo];}

############################################################################

for (pri in shuf) { ku=shuf[pri]; 
...
########    
 ...
 parame=int(wei_FIFRIN*diefact * rand());
...
#   only FI3T (22) and NOT A3T (22) 
 ...
 parame=int(wei_A3TFI3T*diefact * rand());
   ...
   diecont++;
       ...
       sourcens=all_node_id[sourc] "_3";
       targens=all_node_id[targe] "_1";
    
      bubs=all_node_atom[sourc]; bubt=all_node_atom[targe];

    if ( bubs=="FI" && bubt=="T" && targens==targ && sourcens==sour) {  if ( targe in node_block || sourc in node_block ) {zuzuzu=0;} else {
...
# FOET inhibited.
    ...
    bubs=all_node_atom[sour];
    
    if (bubs=="FOE") {parame=1;}
...
#   FOFOE, which appears as move 33. It then blocks the nodes which participate to this move. 
    ...
    if ( bubs=="FO" && bubt=="FOE" && targens==targ && sourcens==sour) { if ( targe in node_block || sourc in node_block ) {zuzuzu=0;} else {
...
#   printf("keepNodesOnTop()\; \n \n \}, nextval())\; \n \n") >> "essy.txt";
 ...
 for (nof in proposed_all_edge_source) { delete proposed_all_edge_source[nof];}
   ...
   for (nof in shuf) { delete shuf[nof];}
