...
# this variant 07.08.2015
#
# forked  from quiner.awk variant 09.05.2015, it removes Arrow elements from the visualization of the initial step
#
...
# selects Arrow elements and their ports, according to the new numbering

    for (rere in all_node_atom ) {

       susuu=all_node_atom[rere];
       susu=all_node_id[rere];       
...
#              invisible[zizi]++; 
              ...
              zezeo=all_edge_out[zeze]; 
              zizio=all_edge_out[zizi]; 
          
              invisible_edge[zezee]++; 
              invisible_edge[zizie]++;
#              invisible_edge[zezeo]++;
#              invisible_edge[zizio]++; 
    ...
    for (r in all_node_atom ) {
     ...
     if (r in invisible ) { zizizizi=0; } else {
     ...
     for(e in all_edge_source ) { 
            ...
            if (e in invisible_edge ) { zizizizi=0; } else {
...
for (nodus in proposed_all_node_atom ) {
 ...
 for (link in proposed_update_edge) {
  ...
  if (link in invisible_edge) { zizizizi=0; } else {
...
for ( nodusi in proposed_node_candi ) {
 ...
 nodb=nodusi;
  ...
  if (nodid in invisible ) { zizizizi=0; } else {
